...
def get_hopper_switch_codes(hopper_id):
    """
    Return BEaM hopper ON/OFF M-codes for the selected hopper.

    Uses M160/M161 etc. because the existing converter and MPF examples
    use M-codes for hopper activation.
    """
    try:
    ...
    if hopper_id not in CONFIG["beam"]["hopper"]:
        raise ValueError("Invalid hopper selection. Choose hopper 1 to 5.")

    hopper = CONFIG["beam"]["hopper"][hopper_id]
    return hopper["on"], hopper["off"]
...
def parse_toolpath_lines(lines, selected_hopper=None, z_hop_threshold=2.0):
    ...
    hopper_on_code = None
    hopper_off_code = None
    if selected_hopper is not None:
        hopper_on_code, hopper_off_code = get_hopper_switch_codes(selected_hopper)
    ...
    for raw_line in selected_lines:
        ...
        if is_motion_to_copy(code_clean):
            ...
            if hopper_on_code and hopper_off_code and current_z is not None and last_z is not None:
                ...
                # If Z rises by more than threshold, switch hopper OFF before the upward travel move.
                if z_delta > z_hop_threshold and not waiting_for_down_move:
                    output.append({"code": hopper_off_code, "comment": None})
                    output.append({"blank": True})
...
def parse_common(lines, source_name="uploaded.HP", power_head="24vx", selected_hopper=None):
    ...
    parsed["selected_hopper"] = int(selected_hopper) if selected_hopper is not None else parsed["hopper"]
    ...
    parsed["toolpath_items"], parsed["toolpath_report"] = parse_toolpath_lines(lines, selected_hopper=parsed["selected_hopper"])
    return parsed

def parse_hp_program(hp_path: Path, power_head="24vx", selected_hopper=None):
    return parse_common(read_text(hp_path).splitlines(), source_name=hp_path.name, power_head=power_head, selected_hopper=selected_hopper)
...
def convert_hp_to_mpf_text(hp_text: str, source_name: str = "uploaded.HP", power_head="24vx", selected_hopper=None) -> str:
    return build_mpf(parse_common(hp_text.splitlines(), source_name=Path(source_name).name, power_head=power_head, selected_hopper=selected_hopper))

def get_conversion_report(hp_text: str, source_name: str = "uploaded.HP", power_head="24vx", selected_hopper=None) -> dict:
    parsed = parse_common(hp_text.splitlines(), source_name=Path(source_name).name, power_head=power_head, selected_hopper=selected_hopper)
    report = parsed["toolpath_report"]
    ...
    return {
        ...
        "selected_hopper": parsed["selected_hopper"],
        "puis_laser": parsed["puis_laser"],
        "unhandled_count": len(report["unhandled"]),
        "unhandled": report["unhandled"][:100],
    }

def convert_hp_to_mpf_file(hp_path: Path, out_path: Path, power_head="24vx", selected_hopper=None):
    out_path.write_text(build_mpf(parse_hp_program(hp_path, power_head=power_head, selected_hopper=selected_hopper)), encoding="utf-8")
    return out_path
...
def main():
    ...
    parser.add_argument("--hopper", type=int, choices=[1, 2, 3, 4, 5], default=None, help="Hopper used for large Z-hop OFF/ON insertion")
    ...
    convert_hp_to_mpf_file(source_path, out_path, power_head=args.power_head, selected_hopper=args.hopper)
